...
class PreguntaController extends Controller
{
   ...
   if ($estatus === 'habilitar') {
    ...
    public function Eliminarversion($formato,$version)
    {
        Pregunta::where('formato', $formato)->where('version', $version)->delete();
        if ((int)$formato === 1) {
    ...
    public function store(StorePreguntaRequest $request)
    {
        ...
        if (in_array($request->formato, [1, 2, 3])) {
            $ultimaPregunta = Pregunta::where('formato', $request->formato)->orderBy('version', 'desc')->first();
            $versions = $ultimaPregunta ? (int)$ultimaPregunta->version + 1 : 1;
        }
