...
class GrupoController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $grupos = Grupo::with('materias', 'profesor.user', 'alumnos.user', 'periodo')
        ->whereHas('periodo', function ($query) use ($fechaActual) {
            $query->where('mes_inicio', '<=', $fechaActual->month) 
                  ->where('mes_fin', '>=', $fechaActual->month);  
        })
        ->whereHas('periodo', function ($query) use ($anioActual) {
            $query->where('año', $anioActual); 
        })
        ->paginate(10);
    ...
    public function alumnos(Request $request): Response
    {
        $user = Auth::user();
        ...
        return Inertia::render("Grupo/Alumno", [
            'titulo' => 'Grupos de ITI',
            'alumnos' => $alumnos,
            'grupos' => $grupos,
            'routeName' => $this->routeName,
    ...
    public function removeAlumno($grupoId, $alumnoId)
{
  ...
  if ($relacion) {
    ...
    public function store(StoreGrupoRequest $request)
    {
        ...
        $periodo_id = Periodo::find($request->input('periodo_id'));
    ...
    {
        $Grupo = Grupo::find($id);
        $grupoAlumnos = Grupo_Alumnos::where('grupo_id', $id)->get();
        
        $materias = Grupo_Materias::where('grupo_id', $id)->get();
        $grupoAlumnosIds = $grupoAlumnos->pluck('id');
        
        $habilitarversiones = Habilitarversiones::whereIn('grupo_alumno', $grupoAlumnosIds)->get();
        $AlumnosAcademico = Academico::where('grupo_id', $id)->get();
        $materiaAlumnosIds = $AlumnosAcademico->pluck('id');
        DB::table('materias_acumuladas')->whereIn('academico_id', $materiaAlumnosIds)->delete();

        foreach ($habilitarversiones as $habilitarversion) {
            $habilitarversion->delete();
        }
        ...
        foreach ($AlumnosAcademico as $academico) {
            $academico->delete();
        }
